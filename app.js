...
var PriceRecordModel = mongoose.model('PriceRecordModelBTC', PriceRecordSchema);
...
app.get('/sim-run-once', function(req, res) {
	...
	PriceRecordModel.find({}, function(error, data){
   		if (error) {
            res.json(error);
        }
        else {
...
function testLoop() {
	...
	result += 'analyzing: ' + btc_data.length + ' values ('+days_in_records.toFixed(2)+' days)<br /><br />';

	for (i=0; i<=(btc_data.length - values_per_period); i++) {
		
		// define start and end indexes for main array
		result += '<strong><u>Period ' + Math.floor((i+values_per_period)/values_per_period) + ' of ' + (btc_data.length/values_per_period).toFixed(2) + ' ';
		result += '(in '+hrs_in_period+' hr periods)</u></strong> ';
		result += '(' + days_in_records.toFixed(2) + ' days) ';
...
function printCurrentPosition(current_coin_price_buy, current_coin_price_sell) {
	...
	result += '&gt; total transactions ' 								+ total_transactions + '<br />';
